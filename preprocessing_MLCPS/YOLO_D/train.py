...
results = model.train(data=".......Preprocessing_MLCPS/yolo_D/config.yaml", epochs=40)
...
input_image_path = "............../Train_Data/D"

# Path to save images that meet the confidence threshold
output_image_path = "............./Output_Conf_Images_D"  # Change to your desired folder
...
for image_file in os.listdir(input_image_path):
    ...
    if len(results[0].boxes) > 0:  # Ensure there is at least one detection
        # Filter boxes with confidence greater than 0.8
        filtered_boxes = [box for box in results[0].boxes if box.conf > 0.83]
        ...
        if filtered_boxes:  # If there are any boxes that satisfy the confidence condition
            ...
            print(f"Saved image with confidence > 0.83: {output_file_path}")
        else:
            print(f"No boxes with confidence > 0.83 for image: {image_file}")
    else:
